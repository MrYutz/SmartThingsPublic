...
def initialize() {
  ...
  log.debug "Subscriptions Complete..."
  
  
  subscribeToCommand(switch1, "on", switchHandler)
  subscribeToCommand(switch1, "off", switchHandler)
  
  log.debug "Subscriptions for Switch 1 Complete..."
  
  subscribeToCommand(switch2, "on", switchHandler)
  subscribeToCommand(switch2, "off", switchHandler)
  
  log.debug "Subscriptions for Switch 2 Complete..."
  
  log.debug "All Initialized..."
}

def switchHandler(evt) {
  log.debug "Dual Relay Adapter -- switchHandler called with event:  name:${evt.name} source:${evt.source} value:${evt.value} isStateChange: ${evt.isStateChange()} isPhysical: ${evt.isPhysical()} isDigital: ${evt.isDigital()} data: ${evt.data} device: ${evt.device}"
...
def rsmHandler(evt) {
  log.debug "rsmHandler called with event:  \nname:${evt.name} source:${evt.source} value:${evt.value} isStateChange: ${evt.isStateChange()} isPhysical: ${evt.isPhysical()} isDigital: ${evt.isDigital()} data: ${evt.data} device: ${evt.device}"
